...
//            }
//
//            // Filter top-rated categories
//            List<String> preferredCategories = categories.entrySet().stream()
//                    .filter(entry -> (Double) entry.getValue() > 3) // Rating threshold
//                    .map(Map.Entry::getKey)
//                    .toList();
//
//            if (preferredCategories.isEmpty()) {
...
//                articlesListView.getItems().clear();
//                for (Document article : recommendedArticles) {
//                    articlesListView.getItems().add(article.getString("Headline"));
//                }
//            });
//        });
//    }
...
//    public void fetchRecommendations(ObjectId userId) {
//        executorService.submit(() -> {
//            System.out.println("Fetching recommendations for user: " + userId); // for debug
//
//            // Find the latest category rated by the user
//            Document latestRating = ratingsCollection.find(new Document("_id", userId))
//                    .sort(new Document("timestamp", -1)) // Assuming a timestamp field exists
//                    .first();
//
//            // for debug
//            System.out.println("Latest rating: " + latestRating);
//
//            if (latestRating != null) {
//                String category = latestRating.getString("category");
//
//                // Fetch articles from the same category
//                List<Document> recommendedArticles = articlesCollection.find(new Document("category", category))
//                        .limit(5)
//                        .into(new ArrayList<>());
//
//                // Update the ListView with recommendations
